    ...
    if (in_array($fileActExt1, $allow) && in_array($fileActExt2, $allow) && in_array($fileActExt3, $allow)) {
        if ($img1['size'] > 0 && $img1['error'] == 0 && $img2['size'] > 0 && $img2['error'] == 0 && $img3['size'] > 0 && $img3['error'] == 0) {
            if (move_uploaded_file($img1['tmp_name'], $filePath1) && move_uploaded_file($img2['tmp_name'], $filePath2) && move_uploaded_file($img3['tmp_name'], $filePath3)) {
                $update_attention = $conn->prepare("UPDATE attention_en SET name1 = :name1, img1 = :img1,name2 = :name2, img2 = :img2,name3 = :name3, img3 = :img3");
                ...
                if ($update_attention) {
                    ...
                    echo "<meta http-equiv='refresh' content='1.5;url=attention_en'>";
                } else {
        ...
        if ($img1['size'] > 0 && $img1['error'] == 0 && $img2['size'] > 0 && $img2['error'] == 0) {
            if (move_uploaded_file($img1['tmp_name'], $filePath1) && move_uploaded_file($img2['tmp_name'], $filePath2)) {
                $update_attention = $conn->prepare("UPDATE attention_en SET name1 = :name1, img1 = :img1,name2 = :name2, img2 = :img2,name3 = :name3");
                $update_attention->bindParam(":name1", $name1);
                $update_attention->bindParam(":img1", $fileNew1);
                $update_attention->bindParam(":name2", $name2);
                $update_attention->bindParam(":img2", $fileNew2);
        ...
        if ($img1['size'] > 0 && $img1['error'] == 0 && $img3['size'] > 0 && $img3['error'] == 0) {
            if (move_uploaded_file($img1['tmp_name'], $filePath1) && move_uploaded_file($img3['tmp_name'], $filePath3)) {
                $update_attention = $conn->prepare("UPDATE attention_en SET name1 = :name1, img1 = :img1,name2 = :name2,name3 = :name3, img3 = :img3");
                $update_attention->bindParam(":name1", $name1);
                $update_attention->bindParam(":img1", $fileNew1);
                $update_attention->bindParam(":name2", $name2);
                // $update_attention->bindParam(":img2", $fileNew2);
        ...
        if ($img1['size'] > 0 && $img1['error'] == 0) {
            if (move_uploaded_file($img1['tmp_name'], $filePath1)) {
                $update_attention = $conn->prepare("UPDATE attention_en SET name1 = :name1, img1 = :img1,name2 = :name2,name3 = :name3");
                $update_attention->bindParam(":name1", $name1);
                $update_attention->bindParam(":img1", $fileNew1);
                $update_attention->bindParam(":name2", $name2);
                // $update_attention->bindParam(":img2", $fileNew2);
        ...
        if ($img2['size'] > 0 && $img2['error'] == 0) {
            if (move_uploaded_file($img2['tmp_name'], $filePath2)) {
                $update_attention = $conn->prepare("UPDATE attention_en SET name1 = :name1,name2 = :name2, img2 = :img2,name3 = :name3");
                $update_attention->bindParam(":name1", $name1);
        ...
        if ($img3['size'] > 0 && $img3['error'] == 0) {
            if (move_uploaded_file($img3['tmp_name'], $filePath3)) {
                $update_attention = $conn->prepare("UPDATE attention_en SET name1 = :name1,name2 = :name2,name3 = :name3, img3 = :img3");
                $update_attention->bindParam(":name1", $name1);
        ...
        $update_attention = $conn->prepare("UPDATE attention_en SET name1 = :name1,name2 = :name2,name3 = :name3");
        $update_attention->bindParam(":name1", $name1);
        $update_attention->bindParam(":name2", $name2);
        $update_attention->bindParam(":name3", $name3);
        $update_attention->execute();
...
//query attention
$attention = $conn->prepare("SELECT * FROM attention_en");
$attention->execute();
            ...
            <section class="section">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h4 class="card-title">ความสนใจภาษาอังกฤษ</h4>
                        <div class="btn-lang">
                            <a href="attention" style="background-color: #DB4834; color: #FFFFFF;" class="btn">TH</a>
                        </div>
