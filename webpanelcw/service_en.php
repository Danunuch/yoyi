...
if (isset($_POST['save_service'])) {
    ...
    foreach ($_FILES['img']['tmp_name'] as $key => $value) {
        ...
        if (in_array($extension, $supported)) {
            ...
            if (move_uploaded_file($_FILES['img']['tmp_name'][$key], "uploads/upload_service/" . $new_name)) {
                $sql = "INSERT INTO service_img_en (image, status) VALUES(:image, :status)";
    ...
    $update_service = $conn->prepare("UPDATE service_en SET area1 = :area1,area2 = :area2,area3 = :area3, area4 = :area4, area5 = :area5,area6 = :area6, area7 = :area7, area8 = :area8");
    ...
    if ($update_service) {
        ...
        echo "<meta http-equiv='refresh' content='1.5;url=service_en'>";
    } else {
        echo "<script>
        $(document).ready(function() {
        Swal.fire({
...
if (isset($_POST['del-img'])) {
    ...
    $del_img = $conn->prepare("DELETE FROM service_img_en WHERE id = :id");
...
//query service_img
$service_img = $conn->prepare("SELECT * FROM service_img_en");
$service_img->execute();
$row_service_img = $service_img->fetchAll();

//query service
$service = $conn->prepare("SELECT * FROM service_en");
$service->execute();
            ...
            <section class="section">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h4 class="card-title">บริการภาษาอังกฤษ</h4>
                        <div class="btn-lang">
                            <a href="service" style="background-color: #DB4834; color: #FFFFFF;" class="btn">TH</a>
                        </div>
                    </div>
                    <script>
