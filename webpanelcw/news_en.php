            ...
            if (in_array($fileActExt1, $allow)) {
                if ($img_cover['size'] > 0 && $img_cover['error'] == 0) {
                    if (move_uploaded_file($img_cover['tmp_name'], $filePath1)) {
                        $add_news = $conn->prepare("INSERT INTO news_en(cover_img,content,link,status) VALUES(:cover_img, :content, :link, :status)");
...
if (isset($_POST['edit_event'])) {
    ...
    if (in_array($fileActExt1, $allow)) {
        if ($img_cover['size'] > 0 && $img_cover['error'] == 0) {
            if (move_uploaded_file($img_cover['tmp_name'], $filePath1)) {
                $edit_news = $conn->prepare("UPDATE news_en SET cover_img = :cover_img, content = :content,link = :link WHERE id = :id");
        ...
        $edit_news = $conn->prepare("UPDATE news_en SET  content = :content,link = :link WHERE id = :id");
        $edit_news->bindParam(":content", $content);
        $edit_news->bindParam(":link", $link);
        $edit_news->bindParam(":id", $news_id);
        $edit_news->execute();
...
if (isset($_POST['change-status'])) {
    ...
    $stmt = $conn->prepare("UPDATE news_en SET status = :status WHERE id =  :id");
...
if (isset($_POST['delete_all'])) {
    if (count((array)$_POST['ids']) > 0) {
        ...
        $del_event = $conn->prepare("DELETE FROM news_en WHERE id in ($all)");
        $del_event->execute();
            ...
            <section class="section">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h4 class="card-title">ข่าวสารภาษาอังกฤษ</h4>
                        <div class="btn-lang">
                            <a href="news" style="background-color: #DB4834; color: #FFFFFF;" class="btn">TH</a>
                        </div>
                        <script>
                                            ...
                                            foreach (array_reverse($row_news) as $row_news) { ?>
                                                ...
                                                <div class="modal fade" id="editevent<?php echo $row_news['id'] ?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                                    <div class="modal-dialog modal-lg modal-dialog-centered">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h1 class="modal-title fs-5" id="staticBackdropLabel">แก้ไขข่าวสารภาษาอังกฤษ</h1>
                                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                            </div>
                                                            <div class="modal-body">
                    ...
                    <div class="modal fade" id="addevent" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                        <div class="modal-dialog modal-lg modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h1 class="modal-title fs-5" id="staticBackdropLabel">เพิ่มข่าวสารภาษาอังกฤษ</h1>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <form method="post" enctype="multipart/form-data">
