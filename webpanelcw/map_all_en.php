...
if (isset($_POST['add_map'])) {
    ...
    if (empty($name)) {
        ...
        try {
            $insert_map = $conn->prepare("INSERT INTO master_map_en(name) VALUES(:name)");
            ...
            if ($insert_map) {
                ...
                echo "<meta http-equiv='refresh' content='1.5;url=map_all_en'>";
            } else {
                echo "<script>
                $(document).ready(function() {
                    Swal.fire({
...
if (isset($_POST['edit_map'])) {
    ...
    $update_map = $conn->prepare("UPDATE master_map_en SET name = :name WHERE id = :id");
...
if (isset($_POST['del_mainmap'])) {
    ...
    $del_main = $conn->prepare("DELETE FROM master_map_en WHERE id = :id");
...
if (isset($_POST['del_store'])) {
    ...
    $del_store = $conn->prepare("DELETE FROM store_en WHERE id = :id");
            ...
            if (in_array($fileActExt1, $allow) && in_array($fileActExt2, $allow)) {
                if ($img1['size'] > 0 && $img1['error'] == 0 && $img2['size'] > 0 && $img2['error'] == 0) {
                    if (move_uploaded_file($img1['tmp_name'], $filePath1) && move_uploaded_file($img2['tmp_name'], $filePath2)) {
                        $insert_store = $conn->prepare("INSERT INTO store_en(name_store,img1,img2,content,date,time,email,phone,fax,master_map) 
...
if (isset($_POST['edit_res'])) {
    ...
    if (in_array($fileActExt1, $allow) && in_array($fileActExt2, $allow)) {
        if ($img1['size'] > 0 && $img1['error'] == 0 && $img2['size'] > 0 && $img2['error'] == 0) {
            if (move_uploaded_file($img1['tmp_name'], $filePath1) && move_uploaded_file($img2['tmp_name'], $filePath2)) {
                $update_store = $conn->prepare("UPDATE store_en SET name_store = :name_store,img1 = :img1,img2 = :img2,content = :content,date = :date,time = :time,
        ...
        if ($img1['size'] > 0 && $img1['error'] == 0) {
            if (move_uploaded_file($img1['tmp_name'], $filePath1)) {
                $update_store = $conn->prepare("UPDATE store_en SET name_store = :name_store,img1 = :img1,content = :content,date = :date,time = :time,
                email = :email,phone = :phone,fax = :fax,master_map = :master_map WHERE id = :id");
        ...
        if ($img2['size'] > 0 && $img2['error'] == 0) {
            if (move_uploaded_file($img2['tmp_name'], $filePath2)) {
                $update_store = $conn->prepare("UPDATE store_en SET name_store = :name_store,img2 = :img2,content = :content,date = :date,time = :time,
                email = :email,phone = :phone,fax = :fax,master_map = :master_map WHERE id = :id");
        ...
        $update_store = $conn->prepare("UPDATE store_en SET name_store = :name_store,content = :content,date = :date,time = :time,
        email = :email,phone = :phone,fax = :fax,master_map = :master_map WHERE id = :id");
...
//query master map
$master_map = $conn->prepare("SELECT * FROM master_map_en");
$master_map->execute();
$row_master_map = $master_map->fetchAll();

//query store
$store = $conn->prepare("SELECT * FROM store_en");
$store->execute();
            ...
            <section class="section">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h4 class="card-title">แผนผังทั้งหมด (ชั้น) ภาษาอังกฤษ</h4>
                        <div class="btn-lang">
                            <a href="map_all" style="background-color: #DB4834; color: #FFFFFF;" class="btn">TH</a>
                        </div>
                    ...
                    <div class="card-body">
                        <div class="mt-4">
                            <div class="mt-2" style="display: flex; justify-content: flex-end;">
                                ...
                                <button type="button" class="btn" data-bs-toggle="modal" data-bs-target="#addrestaurant" style="background-color: #DB4834; color: #FFFFFF; margin-right: 5px;">เพิ่มร้าน</button>
                            </div>
                        </div>
                        <div class="mt-3">
                            <div class="row">
                            ...
                            <div class="pos">
                                ...
                                for ($i = 0; $i < count($row_store); $i++) { ?>
                                    <div class="main-bax-res">
                                        ...
                                        <form method="post">
                                            <button class="btn btn-del-main" type="submit" name="del_store" value="<?php echo $row_store[$i]['id'] ?>" onclick="return confirm('ต้องการลบชั้นนี้ใช่หรือไม่');">ลบ</button>
                                        </form>
                                    </div>
